...
import { LayoutDashboard, Zap, Grid, Settings, HelpCircle, Menu, X } from 'lucide-react';
    ...
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[var(--bg-primary)] border-b border-[var(--border-color)] px-4 flex items-center justify-between z-30">
        <div className="flex items-center space-x-2.5">
          ...
          <span className="font-bold text-sm tracking-tight text-white">Message Now AI</span>
        </div>
      ...
      {/* Sidebar Container */}
      <aside className={`w-72 border-r border-[var(--border-color)] bg-[var(--bg-primary)] flex flex-col justify-between p-6 fixed inset-y-0 left-0 z-50 transition-transform duration-300 ease-in-out ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        }`}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex flex-col">
                <span className="font-bold text-base tracking-tight text-white leading-tight">Message Now AI</span>
                <span className="text-[10px] text-pink-400 font-semibold tracking-wider">Automate Instagram DM</span>
              </div>
            </div>
        ...
        <div className="flex flex-col space-y-6">
          <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-3 flex items-center space-x-3">
            ...
            <div className="flex flex-col overflow-hidden">
              <span className="text-xs font-semibold text-white truncate">{data?.user?.username}</span>
